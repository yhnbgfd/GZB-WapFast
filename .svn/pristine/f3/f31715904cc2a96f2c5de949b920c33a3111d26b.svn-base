...
<%@ include file="Fragment/head.jsp"%>   
 ...
 <div class="title">工资信息</div>
  <div class="p-l-8"><a href="<%=mainurl%>">回到首页</a>&nbsp;|&nbsp;<a href="<%=userurl%>">个人信息</a>&nbsp;|&nbsp;<a href="<%=updateurl%>">修改资料</a><br/>
 ...
 %>
 <div class="p-l-8"><%=errMassage %></div>
 <%} %>
  <div class="p-l-8"><a href="<%=wageurl%>">回到工资列表</a></div>
 <%Wage wage = (Wage)request.getAttribute("wage"); %>
  <div class="special">
  <div class="contentLine"><strong>公司：</strong><%=wage.getFactory()%></div>
  <div class="contentLine"><strong>工资：</strong><%if(wage.getWage()!=null) {%><%=wage.getWage()%><%}else{ %>暂未开始发放<%} %></div>
  <div class="contentLine"><strong>月份：</strong><%if(wage.getDate()!=null) {%><%=wage.getDate()%><%}else{ %>暂未开始发放<%} %></div>
  <div class="contentLine"><strong>详情：</strong><br/><%if(wage.getJSON()!=null) {%><%=wage.getJSON()%><%}else{ %>暂未开始发放<%} %></div>
  <%if(wage.getAgentInfo()!=null&&!wage.getAgentInfo().isEmpty()) {%><div class="p-t-10"><div class="contentLine">工资有任何疑问，请联系</div><div class="contentLine"><%=wage.getAgentInfo()%></div></div><%} %>
  </div>
 <%
 }else{%> 
